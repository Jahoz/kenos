...
class _ContributePanelState extends ConsumerState<_ContributePanel> {
  ...
  /// The door's own refusal, learned at the peek: when the ring went
  /// back to the void (or closed, or the key died) the composer NEVER
  /// shows — nobody types a line the ether has already refused.
  String? _deadReason;
  ...
  /// the whole. The same ask is the door's truth: a KENOS_* refusal
  /// kills the offer honestly (the ring is dead, closed or locked);
  /// anything else is the sky being far — fail-open, the send will
  /// say its own truth.
  Future<void> _peekPrevious() async {
    ...
    try {
      ...
      if (e.toString().contains('KENOS_')) {
        if (mounted) {
          setState(() => _deadReason = contributeRefusalMessage(e));
        }
        return;
      }
      debugPrint('[kenos.constellations] peek unreachable: $e');
    }
    if (mounted) {
      setState(() {
        _previous = previous;
